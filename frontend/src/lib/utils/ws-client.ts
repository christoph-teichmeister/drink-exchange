...
const defaultEvents = [
  ...
  { title: 'Event: Calm Markets', description: 'Liquidity stabilized at 21:14' },
...
export function createMarketWebSocket(barId: string) {
  ...
  const wsUrl = `${(import.meta.env.VITE_WS_BASE_URL ?? 'ws://localhost:8000').replace(/\/+$/, '')}/ws/market/${barId}/`;
  ...
  const startMockFeed = () => {
    ...
    mockTimer = setInterval(() => {
      dispatchPayload('prices.update', {
        rates: defaultPrices.map((rate) => ({ ...rate, price: `${rate.price} +${(index % 5) + 1}%` }))
      });
  ...
  const off = (eventType: string, handler: MarketEventHandler) => {
    const store = eventType.includes('*') ? wildcard.get(eventType) : listeners.get(eventType);
